...
| 维度 | v1.0 | v2.0 |
|------|------|------|
| **编排方式** | 人类写死的 YAML workflow | LLM 动态生成 DAG |
| **Agent 数量** | 1 个 Worker 换 prompt | 多个独立 Worker 实例 |
| **上下文** | 共享，无隔离 | 每个 Agent 独立 |
| **执行模型** | 线性 stage 序列 | DAG 拓扑调度 + 并行 |
| **失败处理** | 硬编码（停止） | 编排 Agent 智能决策 |
| **扩展性** | 改代码 | 注册表配置 |
...
harness/
├── core/
│   ├── models.py                    # v1.0 模型（保留兼容）
│   ├── models_v2.py                 # v2.0 模型（DAG/AgentCapability/...）
│   ├── config.py                    # 配置管理
│   ├── agent_registry.py            # Agent 能力注册表
│   └── dag_engine.py               # DAG 执行引擎
├── orchestrator/
│   ├── engine.py                    # v1.0 编排（保留兼容）
│   └── intelligent_orchestrator.py  # v2.0 智能编排 Agent
├── agent/
│   ├── worker.py                    # v1.0 Agent Worker
│   └── agent_pool.py               # v2.0 Agent 实例池
├── session/
│   └── store.py                     # 事件存储
├── tools/
│   └── registry.py                  # 工具注册
...
v2.0 和 v1.5 不是替代关系，而是**互补**的：

| 维度 | v1.5 项目特化 | v2.0 多 Agent 编排 |
|------|-------------|------------------|
| **目标** | Agent 知道项目的技术栈和规范 | Agent 能自主协作完成复杂任务 |
| **改动层** | Agent Worker（注入项目知识） | Orchestrator（动态规划） |
| **何时用** | 任何项目都需要 | 复杂任务才需要 |
| **依赖关系** | v1.5 是基础 | v2.0 依赖 v1.5（项目特化知识提升规划质量） |

**最佳实践**：先完成 v1.5（项目特化），再接入 v2.0（多 Agent 编排）。项目上下文能让编排 Agent 做出更精准的任务分解。
