    ...
    async def before_execution(self, ctx: ExecutionContext) -> None:
        try:
            ...
            if config.memory.enabled:
                ...
                ctx.memory_manager.run_maintenance()
...
class LearningHook(ExecutionHook):
    ...
    def _init(self) -> None:
        try:
            ...
            optimizer = LearningOptimizer(mm)
            self._scheduler = LearningScheduler(config.learning, analyzer, optimizer)
        except Exception as exc:
...
class ImpactHook(ExecutionHook):
    ...
    def _init(self) -> None:
        try:
            ...
            self._predictor = ImpactPredictor(
    ...
    async def before_execution(self, ctx: ExecutionContext) -> None:
        if self._predictor is None or not ctx.work_dir:
            return
        try:
            impact_project_path = (
            ...
            impact_scope = await self._predictor.predict(
